...
internal static partial class NodesRepository
{
    ...
    )
    {
        ...
        if (config.PluralAutoEntity != PluralAutoEntity.Off)
        {
            ...
            if (isPlural)
            {
                switch (config.PluralAutoEntity)
                {
                    case PluralAutoEntity.Cardinal:
                        return new DetectionResult(DetectionType.PluralCardinal);
                    case PluralAutoEntity.Ordinal:
                        return new DetectionResult(DetectionType.PluralOrdinal);
                    case PluralAutoEntity.Off:
                        break;
                    default:
